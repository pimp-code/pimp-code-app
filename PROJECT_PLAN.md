...
8. **Implementation in progress:** Rust persistence, validation, transition, result-history, interruption, resume, deletion, retention, and project-update tests are implemented. Renderer component integration tests now cover project management, overview, global settings and retention controls, interrupted-job recovery controls, history deletion, and Markdown checklists. Full shell interaction tests and packaged Tauri restart smoke tests for multiple projects and profiles remain.
